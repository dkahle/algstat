...
#' resulting p-value.
#'
#' In many ways, \code{loglinear()} is like [stats::loglin()] or
#' [MASS::loglm()]; however, there are a few key differences.
#'
...
#'
#' The second difference between \code{loglinear} and [loglin()] or [loglm()] is
#' that inference is made through Monte Carlo simulation. In particular,
#' \code{loglinear()} leverages Markov moves to sample from the conditional
#' distribution of the data given its sufficent statistics.  If the software
#' 4ti2 is installed on your machine, you can use [markov()] (or let
#' \code{loglinear()} use [markov()]) to generate a Markov basis to use for the
#' Markov moves. This basis is guaranteed to produce a MCMC routine that
#' converges to the conditional distribution of interest. Since
#' \code{loglinear()} uses Monte Carlo simulation to conduct inference, and
#' since it uses MCMC to do so, concerns typical to MCMC should be addressed. In
...
#'
#' A third way that \code{loglinear()} differs from [loglin()] or [loglm()] is
#' in generalizing the kinds of tests performed.  While those allow for
#' asymptotic unconditional testing using Pearson's X^2 test and the likelihood
#' ratio test, \code{loglinear()} gives several test statistics: Pearson's X^2,
#' the likelihood ratio G^2, Freeman-Tukey, Cressie-Read (lambda = 2/3), and
#' Neyman's modified X^2., see the last reference.  In other words, to compute
#' the exact p-value, iter = 1e4 samples are sampled from the conditional
#' distribution of the table given the sufficient statistics, and then the
#' proportion of tables that have X^2, G^2, etc. values greater than or equal to
#' that of the observed table is the p value for the (conditional) exact test. A
#' similar, and perhaps preferable approach, simply adds up the probabilities of
#' the tables that have probabilities less than or equal to that of the observed
#' table; this is the first line output in hierarchical and does not use a test
#' statistic.
#'
...
#' @author David Kahle
#' @seealso [stats::loglin()], [MASS::loglm()], [metropolis()]
