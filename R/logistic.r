                     ...
                     ...)
{
  ...
  if(modelGivenByMatrix){
    ...
    if(is.formula(model)){ 
      ...
      if(length(model) == 1){
        init  <- unlist(data[vars == response])
        p     <- 1
        data <- data[vars == model]
        
      }else{
        #If model specifiaction, then make table
        data <- supressMessages(teshape(cbind(data[vars == response],data[vars %in% model]), "tab", freqVar = response))
        p <- length(dim(data))
        init <- tab2vec(data)
      }
    } 
    ...
    if(all(unlist(model) %in% vars)){ # variable names      
      ...
      stop("Invalid model specification, see ?poisson")
    }
    
    # make configuration (model) matrix
    pois_A <- pmat(dim(data), facets)
    A <- lawrence(pois_A)
  }
